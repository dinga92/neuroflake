...
in
  pkgs.stdenv.mkDerivation rec {
    ...
    buildInputs = [
      fslBase
      fslArmawrap # ← makes libfsl-armawrap.so available
      fslMiscmaths
      fslMiscTcl
      fslNewimage
      fslNewnifti
    ...
    configurePhase = ''
      ...
      mkdir -p etc/fslconf config include lib
      ln -s ${fslBase}/config/default.mk etc/fslconf/default.mk
      ln -s ${fslBase}/config/rules.mk   etc/fslconf/rules.mk
      ln -s ${fslBase}/etc/fslconf/*.sh  etc/fslconf/
      export FSLCONFDIR=$PWD/etc/fslconf
      source $FSLCONFDIR/fsl-devel.sh

      ln -s ${fslMiscmaths}/include/miscmaths include/
      ln -s ${fslMiscmaths}/lib/libmiscmaths.*  lib/
    '';

    buildPhase = ''
      export FSLDIR=$PWD
      export FSLCONFDIR=$PWD/etc/fslconf
      make FSLDEVDIR=$PWD FSLSWDIR=$PWD FSLCONFDIR=$FSLCONFDIR
    '';

    installPhase = ''
      export FSLDIR=$PWD
      export FSLCONFDIR=$PWD/etc/fslconf
      make install FSLDEVDIR=$PWD FSLSWDIR=$PWD FSLCONFDIR=$FSLCONFDIR
      cp -r . $out
    '';

    meta = with pkgs.lib; {
      description = "FSL FAST (version 4)";
      homepage = "https://git.fmrib.ox.ac.uk/fsl/fast4";
    };
  }
